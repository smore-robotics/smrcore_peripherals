...
- CMakeLists.txt: 顶层构建/安装/package config；`file(GLOB_RECURSE .../src/*.cpp CONFIGURE_DEPENDS)` 自动收集库源；`smrcore_peripherals` 设置 `EXPORT_NAME peripherals`，安装导出目标为 `smrcore::peripherals`；`install(DIRECTORY src/peripherals/ ... *.hpp)` 安装公开头（排除内部 `sample_model.hpp`）；`--with-sdk ON` 时构建 SDK bridge 与 F/T 标定应用；产物 `build_<Type>/bin|lib|install`
...
- app/: 探测、读数、SDK 桥接与 F/T 标定工具，见 `app/_index.md` 与 `app/README.md`
